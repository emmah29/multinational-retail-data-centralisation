...
class MultiNationalReports:
        '''
        MultiNational_Reports
        This is a control class generates the reports
        ...
        def reports():
                ...
                cloned_orders_table['location'] = cloned_orders_table['store_code'].apply(lambda x: 'Web' if x[0:3] == 'WEB' else 'Offline')
                ...
                orders_with_date_and_price = orders_with_date_and_price.loc [ orders_with_date_and_price['last_row'] != 0]
                # - - now create the difference column
                orders_with_date_and_price['time_between_orders'] = \
                        orders_with_date_and_price.apply(lambda row: row['order_datetimestamp'] - row['order_datetimestamp_previous_day'], axis=1)
                # It is now possible to create the report
                grouped_data = orders_with_date_and_price.groupby('year')
